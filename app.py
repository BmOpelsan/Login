...
def login():
    ...
    global login
    global password
    conn = psycopg2.connect(database="bobrs",
                        user="postgres",
                        password="5623",
                        host="localhost",
                        port="5432")
    ...
    if request.method == 'POST':
        if request.form.get("login"):
            ...
            try:
                cursor.execute("SELECT * FROM login WHERE login=%s AND password=%s", (str(login), str(password)))
...
def registration():
    ...
    if request.method == 'POST':
        ...
        if (password == ""):
            ...
            return 'Вы не ввели пароль'
        try:
            cursor.execute("INSERT INTO login (name,login,password) values (%s,%s,%s)", (str(name), str(login), str(password)))
            conn.commit()
            conn.close()
            return redirect("/account/")
        except:
